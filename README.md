...
- **WhatsApp flap:** `connecting` with rising `cycles_10m` = socket drops (redeploy overlap, phone offline, companion killed) — the worker checks health every 10s (socket + login flag + live server round-trip probe, so even half-open dead sockets are caught), retries forever with rising backoff (3s→15s→1m→5m), and recovers alone; steady state is `connected`, `fail_count: 0`. Socket ownership is single-leader via the `whatsapp_leader` row (90s lease, `instance`/`leader` in `/api/whatsapp/status`): extra processes stay standby instead of fighting over the device — but still keep replicas at 1. Every Stopping/Starting Container pair briefly flaps — avoid rapid successive deploys.
- **Pairing:** QR rotates automatically when its ~160s window lapses (never scan a stale code); no camera? `POST /api/whatsapp/pair-code {phone}` gives an 8-char code to type into the phone (Linked devices → Link with phone number). A server-side logout auto-starts a fresh pairing window on its own; only repeated kills (>3/30min, likely ban) park in `logged_out` for a human.
